...
public class ClassService {
	...
	public ClassForm findById(int id) {
		return template.queryForObject("select id, teacher_id teacher, start_date start, months, description from classes where id = :id",
										Map.of("id", id), 
	...
	public ClassDetailsVO findDetailsById(int classId) {
		...
		// Class Info
		var sql = "%s where c.id = :id %s".formatted(SELECT_PROJECTION, SELECT_GROUPBY);
		var classListVO = template.queryForObject(sql,
													Map.of("id", classId),
													new BeanPropertyRowMapper<>(ClassListVO.class));
